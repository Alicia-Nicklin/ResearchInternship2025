# Low-Low analysis - Task 5 from the internship
...
main_folder <- "KM_task5_low_low_analysis"
...
# Settings - using CES1 and CPT1A 
GENES <- c("CES1", "CPT1A")
THRESHOLD <- 0.25  # 25th percentile - the lowest expressers

cat("Low-Low vs Others analysis\n")
cat("Using:", paste(GENES, collapse = " + "), "\n")
cat("Threshold:", THRESHOLD, "(25th percentile - only the lowest expressers)\n")
cat("Data:", nrow(DF), "patients\n\n")

# Method 1: Z-score approach
cat("Creating z-score Low-Low groups...\n")

# Calculate z-scores for each gene
ces1_z <- scale(as.numeric(DF$CES1))[,1]
cpt1a_z <- scale(as.numeric(DF$CPT1A))[,1]

# Set thresholds (25th percentile for each gene)
ces1_thresh <- quantile(ces1_z, THRESHOLD, na.rm = TRUE)
cpt1a_thresh <- quantile(cpt1a_z, THRESHOLD, na.rm = TRUE)

# Create Low-Low group - patients must be low in BOTH genes
...
cat("Creating ssGSEA Low-Low groups...\n")
...
# Low-Low group needs low scores in BOTH pathways
DF$ssgsea_group <- ifelse((metabolic_score < metab_thresh) & (drug_metab_score < drug_thresh), 
                          "Low-Low", "Others")

# Check group sizes
z_low <- sum(DF$zscore_group == "Low-Low", na.rm = TRUE)
z_others <- sum(DF$zscore_group == "Others", na.rm = TRUE)
s_low <- sum(DF$ssgsea_group == "Low-Low", na.rm = TRUE)
s_others <- sum(DF$ssgsea_group == "Others", na.rm = TRUE)

cat("Z-score groups: Low-Low =", z_low, "| Others =", z_others, "\n")
cat("ssGSEA groups: Low-Low =", s_low, "| Others =", s_others, "\n\n")

# Survival analysis function - similar to what I used before
run_low_low_analysis <- function(df, group_col, method_name, endpoint, save_folder) {
  ...
  clean_data <- clean_data[clean_data$group %in% c("Low-Low", "Others"), ]
  ...
  n_low <- as.numeric(group_table["Low-Low"])
  n_others <- as.numeric(group_table["Others"])
  
  if(is.na(n_low)) n_low <- 0
  if(is.na(n_others)) n_others <- 0
  
  if(n_low < 5 || n_others < 5) {
    cat(" FAILED (groups too small)\n")
    return(NULL)
  }
  
  # Make group a factor
  clean_data$group <- factor(clean_data$group, levels = c("Others", "Low-Low"))
  
  # Do the survival analysis
  tryCatch({
    
    # Fit curves for each group
    surv_low <- survfit(Surv(time, event) ~ 1, data = clean_data[clean_data$group == "Low-Low", ])
    ...
    filename <- paste0("CES1_CPT1A_low_low_", safe_method, "_", endpoint, ".png")
    ...
    plot(surv_others, col = "blue", lwd = 2, 
         ...
         main = paste("CES1 + CPT1A Low-Low vs Others -", method_name, 
                      "\n", endpoint, "| p =", round(pval, 4)))
    
    lines(surv_low, col = "red", lwd = 2)
    
    # Add legend
    legend("topright", 
           legend = c(paste("Others (n =", n_others, ")"), 
                      paste("Low-Low (n =", n_low, ")")),
           col = c("blue", "red"), 
           lwd = 2)
...
cat("Running Low-Low survival analyses:\n")

# Test each endpoint for both methods
for(endpoint in c("OS", "DSS", "PFI")) {
  
  # Z-score method
  z_result <- run_low_low_analysis(DF, "zscore_group", "Z-score", endpoint, zscore_folder)
  
  # ssGSEA method
  s_result <- run_low_low_analysis(DF, "ssgsea_group", "ssGSEA", endpoint, ssgsea_folder)
...
if(nrow(results) > 0) {
  ...
  cat("\n==================================================\n")
  cat("LOW-LOW ANALYSIS RESULTS\n")
  cat("==================================================\n\n")
  
  # Show main results
  print(results[, c("Method", "Endpoint", "Low_Low_N", "P_Value", "Significant")])
  
  # Save detailed results
  results_file <- file.path(results_folder, "task5_low_low_results.csv")
  ...
  report_file <- file.path(results_folder, "task5_summary_report.txt")
  sink(report_file)
  cat("LOW-LOW vs OTHERS ANALYSIS SUMMARY\n")
  cat("==================================\n\n")
  cat("Gene pair: CES1 + CPT1A\n")
  cat("Threshold: 25th percentile\n")
  cat("Approach: Low-Low (both genes low) vs Others\n")
...
if(length(z_files) > 0 || length(s_files) > 0) {
  cat("\nTask 5 complete:\n")
  cat("- Low-Low vs Others analysis done\n")
